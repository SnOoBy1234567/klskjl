...
local setPosBtn = Instance.new("TextButton")  -- Yeni buton
...
setPosBtn.Parent = heading
setPosBtn.BackgroundColor3 = Color3.fromRGB(0, 255, 0)
setPosBtn.BorderColor3 = Color3.fromRGB(0, 0, 0)
setPosBtn.BorderSizePixel = 0
setPosBtn.Position = UDim2.new(0.80, 0, 0.15, 0)
setPosBtn.Size = UDim2.new(0, 100, 0, 18)
setPosBtn.Font = Enum.Font.SourceSansBold
setPosBtn.Text = "Set Position"
setPosBtn.TextColor3 = Color3.fromRGB(0, 0, 0)
setPosBtn.TextScaled = true
setPosBtn.TextSize = 14.000
setPosBtn.TextWrapped = true
...
-- Burada hedef CFrame'i kendi istediğin gibi düzenle:
local targetCFrame = CFrame.new(
    -42.6950111, 4.54937553, -28.4859295,
    0.996023297, 1.06597575e-08, -0.0890930966,
    -5.35956657e-09, 1, 5.97297074e-08,
    0.0890930966, -5.90146776e-08, 0.996023297
)

local originalReferencePos = Vector3.new(-27, -20, 0) -- toolGripOffset

-- Tool'ları hedef CFrame'e taşıyan fonksiyon
local function moveToolsToTargetCFrame(targetCFrame)
    local character = player.Character or player.CharacterAdded:Wait()
    for _, tool in ipairs(character:GetChildren()) do
        if tool:IsA("Tool") then
            local gripCFrame = tool.Grip or CFrame.new()
            local offset = gripCFrame.Position - originalReferencePos
            local newGrip = targetCFrame * CFrame.new(offset)
            tool.Grip = newGrip
        end
    end
end
...
local function XMLK_fake_script()
    ...
    local function onInputBegan(input, gameProcessed)
        ...
        if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType.Touch) and
...
-- Yeni butonun işlevi
setPosBtn.Activated:Connect(function()
    moveToolsToTargetCFrame(targetCFrame)
end)
